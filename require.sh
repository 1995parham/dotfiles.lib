...
# remove packages using brew
function not_require_brew() {
  declare -a to_remove_pkg
  to_remove_pkg=()

  for pkg in "$@"; do
    running "not require" " brew ${pkg}"
  ...
  if [[ ${#to_remove_pkg[@]} -ne 0 ]]; then
    action "not require" " brew uninstall ${to_remove_pkg[*]}"
    brew uninstall "${to_remove_pkg[@]}"
  fi
}
